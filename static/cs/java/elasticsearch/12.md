...
```
    ...
    void testBulkDelete() throws IOException {
        ...
        BulkRequest request = new BulkRequest();
        // 2.准备请求参数
        request.add(new DeleteRequest("demo").id("1"));
        request.add(new DeleteRequest("demo").id("2"));
        request.add(new DeleteRequest("demo").id("3"));
        request.add(new DeleteRequest("demo").id("4"));
        request.add(new DeleteRequest("demo").id("5"));
        // 3.发送请求
        client.bulk(request, RequestOptions.DEFAULT);
    }
```

#### 3. 统计数据量： GET /demo/_count
<br/>
